...
#给定比特流，进行封装与发送
def sendbits(data):
    newdata=five_1_pro(data)
    print("处理连续5个1的情况："+str(newdata))
    print("bits位数："+str(len(newdata)))
    datalist=[]
    bits64=[]
    for i in range(len(newdata)):
        if i%64==0 and i!=0:
            datalist.append(bits64)
            bits64=[]
        bits64.append(newdata[i])
    if len(bits64)!=0:
        datalist.append(bits64)
    datalist_len=len(datalist)
    print("分得帧数："+str(datalist_len))
    for i in range(datalist_len):
        datalist[i]=''.join([str(x) for x in datalist[i]])
        datalist[i]=D2odd_addcheck(datalist[i]) #加入校验码
        datalist[i]=add_location(datalist[i])   #加入帧头帧尾
    return datalist
...
def recvbits(data):
    ...
    for i in range(framenum):
        ...
        data[i]=D2odd_check(data[i])
        data[i]=five_1zero_pro(data[i])
    rawdata=''.join(data)
    return rawdata

if __name__=='__main__':
    print("原始数据："+'0000000000')
    newdata=five_1_pro('0000000000')
    print("处理连续5个1的情况："+str(newdata))
    addcheck=D2odd_addcheck(newdata)
    print("加入校验码："+str(addcheck))
    datawithht=add_location(addcheck)
